...
def GSC_adhock(rooted_tree, output_all=False):
    ...
    NOTE: I saw a root-first algorithm in PyCogent that could definitely speed this up but would
    have to consider how that algorithm would play with other downstream applications that use these
    weights (specifically the full_weights_dict).
...
def normalize_GSC_weights(weights_dict, rooted_tree):
    """
    This is a novel (I believe) normalization that I came up with to re-scale GSC weights.
    It simply divides the weight for each terminal by the overall depth of that terminal
    and therefore can/should(?) be thought of as a % independence.

    Input(s):
    weights_dict - weights dictionary, output from "GSC_adhock"
    rooted_tree - a Bio.Phylo tree object that was used as input for "GSC_adhock"
...
def ACL_adhock(rooted_tree):
    ...
    flowing out from each terminal leaf in a tree. It works well, and is pretty fast, but I couldn't figure
    out how to avoid a matrix inversion which should supposedly be possible. Thus, np.linalg.inv makes a 
    clunky showing with all it's associated problems.

    NOTE: I would really like to think of a way to normalize these values as a % independence-like metric
    
    Input(s):
    rooted_tree - a Bio.Phylo tree object that must be properly rooted for the numbers to make any sense
    ...
    rooted_tree - the rooted_tree object because it's possible I have clipped zero branch length clades and/or
            altered the length of the root clade
    """
    assert rooted_tree.is_bifurcating()
    assert rooted_tree.rooted
...
def vcv_recursive(putative_root, vcv_matrix, finished):
    """
    This is a real fast recursive functoin to calculate the variance co-variance matrix. 
    
    Input(s):
    putative_root - the root node (and subsequently, other clades) from a Bio.Phylo tree object
    vcv_matrix - the matrix which will store everything and needs to be n x n where n is the 
            total number of terminals in the tree object where putative_root belongs
    finished - just a list to keep track of everything for record keeping
...
def trim_zero_bls(my_tree):
    """
    Certain weighting schemes (specifically ACL) really do not like zero branch length terminals so this code trims them
    kind of randomly. Am still not entirely sure why this must be the case.
...
def HH_adhock(records_list):
    ...
    Input(s):
    records_list - a list of Bio.Seq objects
